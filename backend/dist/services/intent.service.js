...
// ---------------------------------------------------------------------------
function detectByKeyword(input) {
...
async function detectIntent(input) {
    ...
    let validIntentKeys = Object.keys(INTENT_KEYWORD_MAP);
    try {
        ...
        console.warn("[intent] Could not fetch registry — using static intent keys as fallback");
    }
    // 2. Try AI classification first
    console.log(`[intent] Classifying via Groq: "${input}"`);
    const aiResult = await (0, groq_1.classifyIntent)({
        userInput: input,
        validIntentKeys,
    });
    if (aiResult) {
    ...
    console.warn("[intent] Groq classification failed or returned unknown — falling back to keyword match");
