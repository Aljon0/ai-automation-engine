...
export interface SummaryMetrics {
  ...
  avg_duration_ms: number;     // average ms for completed executions
...
async function getSummaryMetrics(userId?: string): Promise<SummaryMetrics> {
  ...
  if (userId) execQuery = execQuery.eq("user_id", userId);
  ...
  const completedRows = rows.filter(
    (r) => r.completed_at && r.created_at && r.status !== "pending"
  );
...
async function getWorkflowMetrics(userId?: string): Promise<WorkflowMetric[]> {
  const supabase = getSupabaseClient();
  ...
  if (userId) wfQuery = wfQuery.eq("user_id", userId);
  ...
  const grouped = new Map<
    string,
    {
      workflow_name: string;
      intent_key: string;
      rows: { status: string; created_at: string; completed_at: string | null }[];
    }
  >();

  for (const row of data ?? []) {
    const registry = row.workflow_registry as unknown as
  | { workflow_name: string; intent_key: string }
  | null;
...
async function getRecentExecutions(userId?: string): Promise<RecentExecution[]> {
  const supabase = getSupabaseClient();
