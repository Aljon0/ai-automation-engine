...
async function groqRequest(request: GroqRequest): Promise<GroqResponse> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Groq API responded with ${response.status}: ${error}`
      );
    }
...
User: "book me a flight" → unknown`;
  ...
  try {
    ...
    if (validIntentKeys.includes(raw)) return raw;
    ...
    console.error(
      `[groq] classifyIntent failed: ${err instanceof Error ? err.message : String(err)}`
    );
    return null;
  }
}